...
class ValidMethodNameSniff extends AbstractScopeSniff
{
    ...
    protected function processTokenWithinScope(File $phpcsFile, $stackPtr, $currScope)
    {
        ...
        // Naming check.
        if (Common::isLowerSnakeCase($namePart) === false
            && in_array($methodName, $this->allowedPublicMethodNames) === false
        ) {
            $errorData = array($methodName);
            $error     = 'Method "%s" must be snake_case';
            $phpcsFile->addError($error, $stackPtr, 'MethodNotSnakeCase', $errorData);
        }
        ...
        if (($scope === 'private' || $scope === 'protected') && Common::hasUnderscorePrefix($methodName) === false) {
        ...
        if ($scope === 'public' && Common::hasUnderscorePrefix($methodName) === true) {
